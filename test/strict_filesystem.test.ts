...
describe('Strict FileSystem MVCC Scenarios', () => {
  ...
  test('Scenario 2: Massive Concurrent Writes', () => {
    ...
    root.createNested().create(counterFile, '0').commit()
  ...
  test('Scenario 4: Persistence with Active Transactions', () => {
    ...
    // Root 2 (Simulate Crash/Restart before commit)
    ...
    // Root 3
    const root3 = new SyncMVCCTransaction(new FileStrategy())
  ...
  test('Scenario 5: Read Your Own Writes', () => {
    ...
    // Initial: "Initial"
    root.createNested().create(file, 'Initial').commit()
  ...
  test('Scenario 6: Delete-Create Cycle', () => {
    ...
    // V1
    root.createNested().create(cycleFile, 'V1').commit()

    // Old Reader starts here (Snapshot V=1)
    const oldReader = root.createNested()

    // Tx1: Delete V1
    root.createNested().delete(cycleFile).commit()

    // Tx2: Create V2 (Same Key)
    root.createNested().create(cycleFile, 'V2').commit()
  ...
  test('Scenario 8: Repeated Read (Stability)', () => {
    ...
    // External Update 1
    root.createNested().write(file, 'B').commit()
    expect(observer.read(file)).toBe('A') // Still A

    // External Update 2
    root.createNested().write(file, 'C').commit()
    expect(observer.read(file)).toBe('A') // Still A
  ...
  class TestSyncMVCCTransaction extends SyncMVCCTransaction<FileStrategy, string, string> {
  ...
  test('Scenario 9: Garbage Collection Logic', () => {
    ...
    expect(holder.read(file)).toBe('Init')
    ...
    // Holder finishes (commits/rolls back - just needs to be removed from activeTransactions)
    // BUT since we don't have explicit rollback, we commit it or just let it exist?
    // Wait, GC uses 'activeTransactions'. If holder is not committed, it is active.
    // We want to verify it IS cleaned up later.
    // So we need to 'finish' holder.
    holder.commit() // This should remove it from activeTransactions (via _merge)
